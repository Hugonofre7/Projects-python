...
def main():
    ...
    while bytesDownloaded < downloadSize:
        ...
        # Don't print a new line at the end, and immediately flush the
        # printed string to the screen:
        print(barStr, end='', flush=True)
        
        time.sleep(0.2)  # Pause for a little bit:
        
        # Print backspaces to move the next cursor to the line's start:
        print('\b' * len(barStr), end='', flush=True)
        
        
def getProgressBar(progress, total, barWidth=40):
    """Returns a string that can be printed to show a progress bar that has barWidth
    bars and has progress amount out of a total amount."""
