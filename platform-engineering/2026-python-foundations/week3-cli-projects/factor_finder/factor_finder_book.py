...
while True:  # Main program loop.
    ...
    for i in range(1, int(math.sqrt(number)) + 1):
        if number % i == 0:  # If there's no remainder, i is a factor of number.
            factors.append(i)
            factors.append(number // i)  # The complementary factor.
            
    #Covert to a set to get rid of duplicate factors:
    factors = list(set(factors))
    factors.sort()  # Sort the factors in numerical order.
